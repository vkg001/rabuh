...
if (isset($_POST['send_friend_request'])) {
    ...
    if (isset($existing_entry[0])  &&  isset($existing_entry[0]['id'])) {
        switch ($existing_entry[0]['status']) {
            ...
            case "0":
                if ($db->delete('friend_list', "id = " . $existing_entry[0]['id']))
    ...
    if ($_SESSION['MSG_SEEN_UPDATE']  &&  $recent_msg_condition  &&  $_SESSION['CHAT_LIST_LAST_UPDATED'] + 10 >= time()  &&  count($typ) == 0  &&  !$_SESSION['WAS_TYPING']) {
        ...
        $chat = $db->select(
            "messages",
            "*,
            CONCAT(sender, '99999999999999999999999999999999999999', receiver) AS unique_id,
        ...
        foreach ($chat as $slide) {
            ...
            $online_badge = '';
            $now_ts = time();
            $last_ts = strtotime($temp_user_data[0]['last_online']);
            $diff_seconds = max(0, $now_ts - $last_ts);
            $diff_minutes = (int) floor($diff_seconds / 60);
            if ($diff_minutes < 1) {
                $online_badge = '<i class="status-mark online" style="left: 3rem; right: inherit; bottom: 1rem; background-color: #25c925; border-color: #25c925; height: 10px; width: 10px;"></i>';
            } else if ($diff_minutes < 60) {
                $online_badge = '<i class="status-mark online-while-ago" style="left: 3rem; right: inherit; bottom: 1rem; background-color: #ffffff; outline-color: #13f147 !important; border-color: #25c925; height: 10px; width: 10px;"></i>';
            } else if ($diff_minutes < 24 * 60) {
                $online_badge = '<i class="status-mark online-grey" style="left: 3rem; right: inherit; bottom: 1rem; background-color: #a1a1a1; border-color: white; height: 10px; width: 10px;"></i>';
            }
            ...
            <span id="avatar-' . base64_encode(DFENC($slide['id'])) . '">
                <div class="contact border-radius-06 bar chat-list-item click-effect toggleVisibility"  data-refresher="#refresh-chat-spinner-' . base64_encode(DFENC($tile['for'])) . '" data-msg-wrapper="#messages-wrapper-' . base64_encode(DFENC($tile['for'])) . '" data-self="' . (DFENC($tile['for'])) . '" data-msg-container="chat-message-container-' . base64_encode(DFENC($slide['id'])) . '" data-msg-loader="chat-msg-loader-' . base64_encode(DFENC($slide['id'])) . '" data-onclick-hide="#chat-list" data-onclick-fadein="#chat-individual-id-' . base64_encode(DFENC($slide['id'])) . '" data-chat-spinner="#chat-spinner-' . base64_encode(DFENC($slide['id'])) . '" data-init="0" style="justify-content: left; align-items: flex-start; text-align: left; border-bottom: 1px dashed silver; padding-bottom: 5rem; margin-top: 0.1rem; padding-top: 0.2rem;">
                    <div class="pic zoom-on-hover" style="background-image: url(\'' . $tile['profile_pic'] . '\'); margin-top: 0.4rem; margin-left: -4rem;">
                    </div>
                    ' . $unread_count_badge . $online_badge . '
    ...
    $posts = $db->select(
        ...
        "ps.*, usr.is_verified, 
            ...
            usr.name, usr.profile_pic, usr.last_online",
        ...
        AND 
            ...
            AND ps.id NOT IN (SELECT item_id FROM view_stats WHERE viewed_by = '" . USER_ID . "' AND item_type = 'POST' )
    ...
    foreach ($posts as $post) {
        ...
        $now_ts = time();
        $last_ts = strtotime($post['last_online']);
        $diff_seconds = max(0, $now_ts - $last_ts);
        $diff_minutes = (int) floor($diff_seconds / 60);
        ...
        $is_online_ui = "";
        if ($diff_minutes < 5) {
            $is_online_ui = '<i class="status-mark online" style="bottom: 0rem; right: 4rem;"></i>';
        }
                ...
                <div class="col-sm-2" style="padding: 0; margin: 0; padding-left: 0.6rem; padding-top: 0.6rem;">
                    <a href="profile?u=' . DFENC($post['user_id']) . '" style="height: 100% !important; display: inline-block; min-width: 15rem;">
                        <figure class="userIcon" style="left: 0.6rem; position: absolute;">
                            <img src="' . $post['profile_pic'] . '" style="height: 3rem; width: 3rem;" class="zoom-on-hover">
                        </figure>
                        ' . $is_online_ui . '
                                                            ...
                                                            <div class="col-sm-12 top-comment-wrapper">
                                                                <a target="" href="profile?u=' . DFENC($cmt['user_id']) . '" class="d-flex">
                                                                    <figure class="userIcon">
                                                                        <img src="' . $cmt['profile_pic'] . '" class="border-radius-50 zoom-on-hover">
                                                                    </figure>
                            ...
                            <div class="row pb-3 accordion-collapse collapse" id="B' . base64_encode(DFENC($post['id'])) . '">
                                <div class="col-sm-1">
                                    <figure class="userIcon" style="left: 0; position: absolute;">
                                        <img src="' . $user_data['profile_pic'] . '" style="height: 3rem; width: 3rem; border-radius: 50%;" class="zoom-on-hover">
                                    </figure>
                                </div>
        ...
        if ($notif['image'] != "") {
            $new_notification_data .= '
                <div class="notification-icon">
                    <img src="' . $notif['image'] . '" alt="" class="zoom-on-hover">
                </div>';
        }
        $new_notification_data .= '
