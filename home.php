...
<body style="max-width: 100%; overflow-x:clip;background: #f4f4f4;">
                                ...
                                <div class="col-sm-2" style="padding: 0; margin: 0; padding-left: 0.6rem; padding-top: 0.6rem;">
                                    <a href="profile?u=<?php echo DFENC($post['user_id']) ?>" style="height: 100% !important; display: inline-block; min-width: 15rem;">
                                        <figure class="userIcon" style="left: 0.6rem; position: absolute;">
                                            <img src="<?php echo $post['profile_pic'] ?>" style="height: 3rem; width: 3rem;" class="zoom-on-hover">
                                        </figure>
                                        <?php
                                        $now_ts = time();
                                                            ...
                                                            <div class="col-sm-12 top-comment-wrapper">
                                                                <a target="" href="profile?u=<?php echo DFENC($cmt['user_id']) ?>" class="d-flex">
                                                                    <figure class="userIcon">
                                                                        <img src="<?php echo $cmt['profile_pic'] ?>" class="border-radius-50 zoom-on-hover">
                                                                    </figure>
                    ...
                    <div class="card-body">
                        <div class="center friend-list-holder-wrapper" style="position: absolute; left: -2.34rem; top: -1.1rem; transform: scale(0.78); height: 45rem;">
                            <div class="friend-list-holder" id="chat-list" style="top: 0rem; justify-content: start; left: 3.4rem;padding-top: 1rem;width: 25rem;">
                                ...
                                (SELECT last_online FROM register WHERE id = sender) AS sender_last_online,
                                (SELECT last_online FROM register WHERE id = receiver) AS receiver_last_online,
    ...
    <?php require_once "./assets/js/nudes_analyser.php"; ?>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".post-wrapper").each(function() {
                let height_str = $(this).css("height");
                height_str = parseFloat(height_str.substring(0, height_str.length - 2));

                max_card = (max_card > height_str) ? max_card : height_str;
            });

            max_card *= 3;

            let wrapper_height = $("#posts-and-story-wrapper").css("height");
            let initial_height;
            initial_height = wrapper_height = parseFloat(wrapper_height.substring(0, wrapper_height.length - 2));

            let refresh_posts = true;
            let data_available = true;
            wrapper_height -= max_card;
            $(document).scroll(function() {
                let current_height = $(this).scrollTop();
                if (current_height > (wrapper_height)) {
                    if (refresh_posts && data_available) {
                        $.ajax({
                            url: "home_helper",
                            method: "POST",
                            data: {
                            ...
                            beforeSend: function() {
                                refresh_posts = false;
                                $("#load-more-post-spinner").removeClass("d-none");
                                $("#load-more-post-icon").addClass("d-none");
                                console.log("request send");
                            },
                            success: function(data) {
                                $("#load-more-post-spinner").addClass("d-none");
                                $("#load-more-post-icon").removeClass("d-none");
                                let res;
                                ...
                                try {
                                    ...
                                    console.log(e);
                                    console.log(data);
                                    Swal.fire("Server not reachable", "", "error");
                                    return;
                                }

                                $("#posts-and-story-wrapper").append(res.data);
                                if (res.data_end) {
                                    data_available = false;
                                    $("#load-more-post-icon").addClass("d-none");
                                }

                                wrapper_height += initial_height;
